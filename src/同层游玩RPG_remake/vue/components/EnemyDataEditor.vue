...
// Props
interface Props {
  enemyData: BattleParticipantExtended | null;
  battleState: any;
}
...
// 计算属性
const hasEnemyData = computed(() => !!props.enemyData);

// 初始化编辑数据
const initializeEditingData = () => {
  if (props.enemyData) {
    editingData.value = {
      id: props.enemyData.id,
      name: props.enemyData.name,
      hp: props.enemyData.hp,
      maxHp: props.enemyData.maxHp,
      level: props.enemyData.level,
      stats: { ...props.enemyData.stats },
      skills: [...(props.enemyData.skills || [])],
    };
  }
};
...
const updateField = (field: string, value: any) => {
  ...
  if (field.startsWith('stats.')) {
    ...
    if (!editingData.value.stats) {
      editingData.value.stats = {};
    }
    editingData.value.stats[statField] = value;
  } else {
    (editingData.value as any)[field] = value;
  }

  // 发送更新事件
  if (eventBus && props.enemyData) {
    eventBus.emit('battle:debug-update-enemy', {
      participantId: props.enemyData.id,
      updates: { [field]: value },
    });
  }
};
...
const resetToInitial = () => {
  ...
  if (eventBus && props.enemyData) {
    eventBus.emit('battle:debug-reset-enemy', {
      participantId: props.enemyData.id,
    });
  }
};
...
<template>
  <div v-if="hasEnemyData" class="enemy-data-editor">
