...
├── services/                # 服务层（18个核心服务）
│   ├── StatDataBindingService.ts # 统计数据绑定
│   ├── GlobalStateManager.ts # 全局状态管理
│   ├── SaveLoadManagerService.ts # 存档管理服务
│   ├── SameLayerService.ts  # 同层聊天服务
│   ├── GameStateService.ts  # 游戏状态服务
│   ├── UIService.ts         # UI服务
│   ├── BattleService.ts     # 战斗协调器
│   ├── BattleEngine.ts      # 战斗计算引擎
│   ├── BattleConfigService.ts # 战斗配置管理
│   ├── BattleConfigInitializer.ts # 战斗配置初始化
│   ├── BattleResourceService.ts # 战斗资源验证
│   ├── BattleResultHandler.ts # 战斗结果处理
│   ├── DynamicEnemyService.ts # 动态敌人生成
│   ├── PhaserManager.ts     # Phaser管理器
│   ├── HistoryManager.ts    # 历史管理（战斗调试）
