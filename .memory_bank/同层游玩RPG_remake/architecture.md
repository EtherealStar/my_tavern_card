...
- **GlobalStateManager**: 全局状态管理，统一管理跨组件、跨环境的全局状态，提升类型安全
...
- **useGlobalState**: 全局状态管理，统一管理跨组件全局状态，提供类型安全的接口
...
- **usePlayingUI**: 游戏UI管理，处理游戏界面交互
- **useCharacterCreation**: 角色创建，管理创建流程的状态和数据
- **useGameSettings**: 游戏设置，管理游戏配置和用户偏好
- **useBattleSystem**: 战斗系统编排器，统一处理战斗流程
- **useBattleState**: 战斗状态真相源，提供响应式战斗数据
- **useBattleConfig**: 战斗配置管理，处理战斗配置加载
- **usePhaserBattle**: Phaser桥梁，管理Phaser游戏实例
- **useBattleAnimation**: 战斗动画管理
- **useCommandQueue**: 指令队列管理，处理批量操作
...
- **GlobalStateManager**: 全局状态管理服务，统一管理跨组件、跨环境的全局状态
- **SaveLoadManagerService**: 存档管理服务，整合了原多个存档相关服务，支持 IndexedDB 和 MVU 变量
- **SameLayerService**: 同层聊天服务，整合了原 TavernGenerationService 功能

### GlobalStateManager 全局状态管理系统

#### 系统概述

GlobalStateManager 是一个统一的全局状态管理服务，用于管理跨组件、跨环境（Vue/Phaser）的全局状态，提升类型安全性和可维护性。
...
1. **待处理存档数据** (PendingSaveData)
   - 用于 StartView 读档后传递数据到 PlayingRoot
   - 支持设置、获取、清除操作

2. **流式生成控制** (StreamHandle)
   - 管理当前流式生成的控制句柄
   - 支持停止流式生成操作
...
export function useMyComposable() {
  ...
  // 存储数据
  globalState.setPendingSaveData(data);
